...
final class SessionDirectImpl implements SqlSession {
     ...
     *
     * @param sql  动态sql 例: select * from user where id=? and name=?
    ...
    public <T> int update(T entity, Serializable... primaryKey) throws SqlSessionException {
        ...
        StringBuilder SQL = new StringBuilder();
        SQL.append(updatesql.getSql()).append(pks.get(0)).append(" ").append("IN").append(" ").append("(");
        ...
        String sql = SQL.append(")").toString();
        List values = updatesql.getValues();
        try {
            pstm = conn.prepareStatement(sql);
            pstmSetListValues(pstm, values);
            for (int i = 0; i < primaryKey.length; i++) {
                pstm.setObject(values.size() + (i + 1), primaryKey[i]);
            }
     ...
     */
    @Override
    public <T> int update(T entity) throws SqlSessionException {
        this.open();
        ...
        try {
            pstm = conn.prepareStatement(sql);
            pstmSetListValues(pstm, values);
            pstm.setObject(values.size() + 1, updatesql.getPkValue());
            values.add(updatesql.getPkValue());
            return pstm.executeUpdate();
        } catch (SQLException e) {
            throw new SqlSessionException("sql执行出错了... ", e);
        } finally {
    ...
    public <T> int updateByWhere(T entity, String where, Serializable... args) throws SqlSessionException {
        ...
        try {
            ...
            for (int i = 0; i < args.length; i++) {
                pstm.setObject(values.size() + (i + 1), args[i]);
            }
            values.add(args);
            return pstm.executeUpdate();
        } catch (SQLException e) {
     ...
     *
     * @param cls   查询指定对象类
     * @param where sql 后续条件: (where(可有可无,处理时默认去掉自带where关键字)) id>10086 and id<10089
     *              order by id desc
     ...
     *
     * @param cls         要返回的对象类
     * @param primaryKeys 该对象的唯一主键（可传多个 查询多个） 注: 如果该对象有多个主键，则默认取第一主键作为此次查询的标识
     * @return
     * @throws SqlSessionException
     */
    @SuppressWarnings("rawtypes")
