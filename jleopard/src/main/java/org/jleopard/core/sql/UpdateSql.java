...
public class UpdateSql<T> implements Sql, CloumnNames, CloumnValue {
	...
	public UpdateSql(T entity) {
		...
		FieldFilterUtil.filterListFeild(columnField, map).forEach((c, v) -> {
			if (c.equals(CollectionUtil.isNotEmpty(FieldUtil.getPrimaryKeys(entity.getClass()))
					? FieldUtil.getPrimaryKeys(entity.getClass()).get(0)
					: null)) {
				this.pkValue = v;
			}else {
				columns.add(c);
	...
	/**
	 *
	 * @return UPDATE USER SET NAME=?,PHONE=? WHERE
	 */
	@Override
